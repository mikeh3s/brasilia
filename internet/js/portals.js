...
var mid = "0701";//主页页面编号 //Número de página de inicio
...
$(function(){
    ...
    //页面曝光上报 Informe de exposición de la página
    // pageViewHomeUploadData();

    /*跳转到主页  Saltar a la página de inicio.*/
    // Actualizar la pagin en cuanto a las estadistica
    ...
    //         resType:resType,
    //         click:1,
    //         flag:1//资源类型为菜单（0：普通资源，1：菜单，2：广告）El tipo de recurso es menú (0: recurso normal, 1: menú, 2: anuncio)
    //     }, function() {
    //         /*if(userInfo){
    ...
    request.success(function(data) {//加载成功则需要加载广告
        ...
        var adSwiper = new Swiper('#adSlide',{
            ...
            onInit: function(swiper){//广告初始化时第一张上报
                ...
                //广告浏览上报  Navegación publicitaria
                $.hongdian.uploadData({
                    resId:resId,
                    resName:resName,
        ...
        $.each(data, function(idx,menu){
            if(idx<0){
                contentR += '<div href="' + menu.href + '" ' +
                    'resId="' + menu.id + '" resName="' + menu.title + '" class="content-links" id="home-' + menu.id + '"> ' +
                    '<img class="img-page" src="' + menu.img + '"/> ' +
                    '</div>';
            }
            else if(idx == 2){
                contentB += '<div id="home-' + menu.id + '" href="' + menu.href + '" ' +
                    'resId="' + menu.id + '" resName="' + menu.title + '" class="content-links col-xs-5 modul-padding-right" > ' +
                    '<img class="img-page" src="' + menu.img + '"/> ' +
                    '</div>';
            }
            else{
                contentB += '<div id="home-' + menu.id + '" href="' + menu.href + '" ' +
                    'resId="' + menu.id + '" resName="' + menu.title + '" class="content-links col-xs-7 modul-padding-left"> ' +
                    '<img class="img-page" src="' + menu.img + '"/> ' +
                    '</div>';
            }
        });
